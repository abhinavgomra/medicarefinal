...
import { logout, isAuthenticated, isDoctor, isAdmin } from '../utils/auth';
...
const Navbar = () => {
  ...
  const admin = isAdmin();
  const doctorMenuPaths = new Set(['/doctor-portal', '/telemedicine', '/profile']);
  const homePath = doctor ? '/doctor-portal' : admin ? '/admin/pharmacy' : '/';
  const canSearchDoctors = !doctor && !admin;
    ...
    <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur supports-[backdrop-filter]:bg-white/90 border-b border-gray-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-3 py-3">
          <Link to={homePath} className="text-2xl font-bold text-primary-700 flex items-center shrink-0">
        ...
        {isOpen && (
          <div className="md:hidden pb-3 mb-3 space-y-2 rounded-2xl border border-gray-100 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/95 shadow-md">
            ...
            <div className="pt-1">
              {routes
                .filter(r => (r.protected ? authed : true))
                .filter(r => (r.doctorOnly ? doctor : true))
                .filter(r => (r.adminOnly ? admin : true))
                .filter(r => (doctor ? doctorMenuPaths.has(r.path) : true))
                .map((r) => (
                  <Link
                    key={r.path}
                    to={r.path}
